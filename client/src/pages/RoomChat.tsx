             ...
             {msg.userId !== user?.uid && (
                <Link href={`/profile/${msg.userId}`}>
                    <a className="cursor-pointer">
                        <img src={msg.photoURL || "https://avatar.vercel.sh/fallback.png"} alt={msg.displayName} className="w-10 h-10 rounded-full" />
                    </a>
                </Link>
             )}
            <div className={`flex flex-col ${msg.userId === user?.uid ? 'items-end' : 'items-start'}`}>
              <div className={`max-w-md p-3 rounded-2xl ${msg.userId === user?.uid ? 'bg-blue-600 text-white rounded-br-none' : 'bg-white dark:bg-gray-800 text-gray-900 dark:text-white rounded-bl-none'}`}>
                <div className="flex items-center gap-2 mb-1">
                    <Link href={`/profile/${msg.userId}`}>
                        <a className="font-semibold text-sm cursor-pointer hover:underline">{msg.displayName}</a>
                    </Link>
