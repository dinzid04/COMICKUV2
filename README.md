...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    match /users/{userId} {
        // Siapa saja bisa membaca profil publik
        allow read: if true;
        // Pengguna hanya bisa menulis ke profilnya sendiri, atau admin bisa menulis ke profil siapa pun
        allow write: if request.auth != null && (request.auth.uid == userId || exists(/databases/$(database)/documents/admins/$(request.auth.uid)));
    }

    // Aturan untuk sub-koleksi PRIBADI (favorit, riwayat, dll.)
    match /users/{userId}/{subcollection} {
        // Hanya pemilik akun yang dapat mengakses sub-koleksi pribadinya
        allow read, write: if request.auth != null && request.auth.uid == userId;
    }

    // Aturan SPESIFIK untuk sistem follow
    match /users/{targetUserId}/followers/{followerUserId} {
        // Siapa saja yang login bisa membaca daftar pengikut
        allow read: if request.auth != null;
        // Pengguna hanya bisa menambahkan/menghapus dirinya sendiri dari daftar pengikut orang lain
        allow write: if request.auth != null && request.auth.uid == followerUserId;
    }

    match /users/{userId}/following/{followingUserId} {
        // Hanya pemilik akun yang bisa membaca daftar 'following'
        allow read: if request.auth != null && request.auth.uid == userId;
        // Pengguna hanya bisa menambahkan/menghapus orang lain ke daftar 'following' miliknya sendiri
